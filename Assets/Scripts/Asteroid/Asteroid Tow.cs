...
public class AsteroidTow : MonoBehaviour
{
    ...
    private WaitForSeconds WaitForSeconds_Joint_Break = new WaitForSeconds(AsteroidData.Joint_Break_Delay_Duration);
    ...
    private void Joint_Velocity_Check()
    {
        if (Player_RigidBody.velocity.magnitude > Joint_Break_Velocity_Player && Is_Tow_Joint_Broke == false)
        {
           ...
           StartCoroutine(Delay_Break_Joint());
    ...
    private IEnumerator Delay_Break_Joint()
    {
        yield return WaitForSeconds_Joint_Break;
        Break_Tow_Joint();
    }
